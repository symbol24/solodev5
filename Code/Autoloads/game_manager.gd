...
func _clear_tower() -> void:
	if self != null:
		...
		if player_manager != null:
			player_manager.queue_free.call_deferred()
			player_manager = null
		player_ready = false
		if currency_manager != null:
			currency_manager.queue_free.call_deferred()
			currency_manager = null
		cm_ready = false
		if level_up_manager != null:
			level_up_manager.queue_free.call_deferred()
...
func _check_all_ready() -> void:
	if tower_ready and player_ready and cm_ready and lup_ready:
